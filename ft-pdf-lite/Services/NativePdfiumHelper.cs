...
namespace FtPdfLite.Services
{
    public static class NativePdfiumHelper
    {
            ...
            lock (_lock)
            {
                ...
                try
                {
                    ...
                    if (!string.IsNullOrEmpty(extractedPath) && File.Exists(extractedPath))
                    {
                        ...
                        // Também inclui no PATH do processo
                        ...
                        // Pré-carrega a biblioteca no processo
                        IntPtr handle = LoadLibrary(extractedPath);
                        if (handle == IntPtr.Zero)
                        {
                            int error = Marshal.GetLastWin32Error();
                            // Se der erro 126 (dependência ausente, ex: Visual C++ Runtime)
        ...
        {
            // 1. Verificar se já existe na pasta atual ou subpasta x64
            ...
            // 2. Destino persistente em %LOCALAPPDATA%\FtPdf\Native\x64\pdfium.dll
            ...
            // Tentar extrair do EmbeddedResource
            try
            {
                ...
                if (resourceName != null)
                {
                    ...
                    if (stream != null)
                    {
                        // Se o arquivo alvo não existe ou tem tamanho diferente
                        ...
                        // Opcional: tenta copiar também para o BaseDirectory\x64 para compatibilidade perfeita com PdfiumViewer
                        try
                        {
